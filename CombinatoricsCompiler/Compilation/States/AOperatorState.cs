...
internal class AOperatorState : CompilationState
{
    public AOperatorState(Compiler source) : base(source)
    {
    }
    ...
    public override string Evaluate()
    {
        ...
        if (long.TryParse(number[0], out var n) == false)
            throw new ArgumentException($"A operator expects long value, instead {number[0]}");

        if (long.TryParse(number[1], out var m) == false)
            throw new ArgumentException($"A operator expects long value, instead {number[1]}");

        if (n < m)
            (n, m) = (m, n);

        return Combinatorics.CombinationRepeating(n, m).ToString();
    }
